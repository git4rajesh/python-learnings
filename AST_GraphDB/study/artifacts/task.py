...
class Task(Base):
    ...
    def _multipletasks(self, task_lst):
        ...
        for task in task_lst:
            self.create_under_project(task.get('title'), task.get('type', 'noncap_nonbill'))
        return self

    def child_task(self, title=None, parent=None, **data):
        """
    ...
    def _get_task(self, title):
        task_dict = self.data_store.search.get_details_based_on_attributes(self.data_store.get(), ENTITY, title=title)
        return task_dict
    ...
    def set_duration(self, days_in_sec):
        ...
        :param days_in_sec: days in secs like 1/2/3
        :return:
        """

...
class Base(Entity):
    ...
    def create(self, **data):
        """
        Portfolio can be created via api
        ...
        url = api.format(
            ...
            category_id=self.constants['PORTFOLIO']['CATEGORIES']['CATEGORY1']['ID'])
    ...
    def read(self, data):
        """
        To get the details of a given portfolio_id
        :param data: data = {entity_id:<<portfolio_id>>}
        :return:
        """
    ...
    def read_category(self):
        """
        To get all details of the portfolio categories
        :return: api response
        """
    ...
    def read_title(self, title, class_id):
        """
        To get the details of a given portfolio_title
        :param title:  Title of the portfolio to fetch
        :return: api response
        """
        api = self.urls[ENTITY]['read_title']
        title = utils.html_decode(title)
        url = api.format(
            protocol='https',
            env=self.cmd_options['env'],
            title=title,
    ...
    def read_finance(self, data):
        """
        To get the details of a given portfolio_title
        :param title:  Title of the portfolio to fetch
        :return: api response
        """
        api = self.urls[ENTITY]['read_finance']
        url = api.format(
            protocol='https',
            env=self.cmd_options['env'],
            entity_id=data['entity_id'])
    ...
    def verify_create(self, response):
        ...
        """
        self.status = True
        self.step_desc = 'Innotas Portfolio create verification'
        ...
        if response.status_code == 201:
            ...
            if read_response.status_code == 200:
                self.remarks += 'Portfolio is created'
            else:
                self.status = False
                self.remarks += 'portfolio creation failed \n failure traceback : {}'.format(
                    response.text)
        else:
            self.status = False

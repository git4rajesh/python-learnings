...
def test_import_similar_title_standard_fieldmapping(request):
    ...
    Fluent(request). \
        data_import().create_for_entity('PROJECT') \
        .with_default_status('PROPOSED').with_default_category('CATEGORY1').where_field_mapping_is('SIMILAR') \
        .include_only_standard_fields() \
        .populate_dynamic_data_to_excel_file(no_of_rows=5) \
        .import_file().verify()
...
def test_import_manual_standard_fieldmapping(request):
    ...
    Fluent(request). \
        data_import().create_for_entity('PROJECT') \
        .with_default_status('PROPOSED').with_default_category('CATEGORY1').where_field_mapping_is('MANUAL') \
        .include_only_standard_fields() \
        .populate_dynamic_data_to_excel_file(no_of_rows=5) \
        .import_file().verify()
...
def test_import_with_udf_fields(request):
    ...
    Fluent(request). \
        data_import().create_for_entity('PROJECT') \
        .with_default_status('PROPOSED').with_default_category('CATEGORY1').where_field_mapping_is('SAME') \
...
@pytest.mark.parametrize('string_comb',
                         ['abcd&s', 'a<b==&"s{>ss!s}d{', 'a&b', '^&', '5&', 'a>b', 'a<b', '5>b', '6<3', '%$&*', 'a\'b',
                          'a"b', 'a+b', 'a@b'])
@zid('576')
...
def test_import_select_list(request):
    """
    Validate data import for creation of two projects having &(ampersand) in values of the list field types
    """
    Fluent(request). \
        data_import().create_for_entity('PROJECT') \
        .where_field_mapping_is('SAME').include_both_standard_and_udf_fields() \
